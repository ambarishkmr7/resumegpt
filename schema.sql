-- ============================================================
-- resumesgpt — MySQL Database Schema
-- 
-- Step 1: Create database (run as root):
  CREATE DATABASE IF NOT EXISTS resumesgpt_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
--
...
INSERT IGNORE INTO cms_pages (id, slug, title, content, icon) VALUES
    (REPLACE(UUID(), '-', ''), 'about-us', '🏢 About Us', 'resumesgpt is an AI-powered resume building platform.', '🏢'),
    (REPLACE(UUID(), '-', ''), 'disclaimer', '⚠️ Disclaimer', 'The information provided by resumesgpt is for general informational purposes only.', '⚠️'),
    (REPLACE(UUID(), '-', ''), 'contact-us', '📬 Contact Us', 'Email: support@resumesgpt.in', '📬'),
    (REPLACE(UUID(), '-', ''), 'faq', '❓ FAQ', 'Q: Do I need to pay to edit?\nA: No! Editing is free.', '❓'),
    (REPLACE(UUID(), '-', ''), 'feedback', '💬 Feedback', 'Share your experience at: support@resumesgpt.in', '💬'),
    (REPLACE(UUID(), '-', ''), 'subscription', '👑 Subscription', 'Pro — ₹299 | Elite — ₹1,999 | One-time lifetime payments.', '👑'),
    (REPLACE(UUID(), '-', ''), 'whats-new', 'Keep you posted', '🚀'),
...
--   import bcrypt
--   bcrypt.hashpw(b"RKambTT#$(28)", bcrypt.gensalt()).decode()

INSERT INTO users (id, email, full_name, hashed_password, is_admin, created_at)
VALUES (
    'admin_seed_001',
    'admin@ResumesGPT.com',
    'Admin',
    '$2b$12$eILtT2vLhc8xYbNgzSJmJ.P8h7ILROKlZIbHOzgpqD3s00OKgevAu',
    TRUE,
    CURRENT_TIMESTAMP
) ;